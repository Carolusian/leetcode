...
def countCharacters(words: List[str], chars: str) -> int:
    ...
    for w in words:
        ...
        characters = chars
        for c in w:
            idx = characters.find(c)
            if idx > -1:
                l += 1
                characters = characters[0: idx] + characters[idx + 1:]
        if l == len(w):
            ret.append(len(w))
    return sum(ret)
